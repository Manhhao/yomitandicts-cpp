...
```
Imports a Yomitan `.zip` dictionary file into a SQLite database. The resulting database is stored in `output_dir/<dict_title>.db`. Glossaries are compressed using zstd. Only term and frequency dictionaries were tested.
...
```
Queries all added databases for the given expression. TermResult includes glossary and frequency data in the order dictionaries were added. Glossaries are decompressed.
